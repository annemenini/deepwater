    ...
    if summaries:
        with tf.name_scope("summaries"):
            for i, (c1, c2) in enumerate(convs):
                ...
                tf.summary.scalar('summary_min_conv_%02d_01' % i, tf.reduce_min(c1))
                tf.summary.scalar('summary_max_conv_%02d_02' % i, tf.reduce_max(c2))

            for k in pools.keys():
                tf.summary.image('summary_pool_%02d' % k, get_image_summary(pools[k]))
                tf.summary.scalar('summary_min_pool_%02d' % k, tf.reduce_min(pools[k]))
                tf.summary.scalar('summary_max_pool_%02d' % k, tf.reduce_max(pools[k]))
